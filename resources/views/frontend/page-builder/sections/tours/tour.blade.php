...
@else
    @php
        ...
        if ($content->box_type === 'normal_with_background_color') {
            $backgroundColor = $content->normal_background_color ?? null;
        } elseif ($content->box_type === 'slider_carousel_with_background_color') {
            $backgroundColor = $content->slider_carousel_background_color ?? null;
        }
        ...
        if (!empty($content->show_only_featured_items)) {
        ...
        if ($content->filter_type === 'custom') {
            ...
            if (!empty($content->filter_category_id)) {
                $categoryIds = (array) $content->filter_category_id;
                $toursQuery->whereHas('categories', function ($query) use ($categoryIds) {
                    $query->whereIn('tour_categories.id', $categoryIds);
                });
            }

            if (!empty($content->filter_city_id)) {
