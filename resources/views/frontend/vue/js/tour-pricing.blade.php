@php
    ...
    $normalTourData = $tour->normalPrices->mapWithKeys(function ($price) use ($firstOrderCoupon) {
        ...
        return [
            formatNameForInput($price->person_type) => [
                ...
                'min' => (int) ($price->min_person ?? 0),
                'max' => (int) ($price->max_person ?? 200),
                'quantity' => (int) ($price->min_person ?? 0),
            ],
        ];
    });
    ...
    $privateTourData = $tour->privatePrices
        ? [
            'min_person' => (int) ($tour->privatePrices->min_person ?? 0),
            'max_person' => (int) ($tour->privatePrices->max_person ?? 200),
            ...
            'quantity' => (int) ($tour->privatePrices->min_person ?? 0),
        ]
        : null;
        ...
        setup() {
            ...
            const totalPriceWithoutCoupon = computed(() => {
                ...
                if (priceType === "promo") {
                    ...
                    if (totalPromoQty === 0) return 0;

                    promoTourData.value.forEach((item) => {
                        // Always use discounted_price (ignore promo_discounted_price)
                        ...
                        if (item.source === 'addon') {
                            ...
                            if (item.type === 'timeslot' && Array.isArray(item
                                .selected_slots)) {
                                item.selected_slots.slice(0, item.quantity).forEach((
                                    slotTime) => {
                                        const slot = item.slots.find((s) => s.time ===
                                            slotTime)
                                        if (slot) {
                                            sum += parseFloat(slot.discounted_price) || 0
                                        }
                                    })
                            }
                        }
                ...
                if (priceType === "normal") {
                    sum += Object.values(normalTourData.value).reduce(
                        (total, {
                            original_price,
                            quantity
                        }) => {
                            return total + parseFloat(original_price) * quantity
                        },
                        0
                    )
                }
                ...
                if (priceType === "water") {
                    Object.entries(waterTourData.value).forEach(([slot, data]) => {
                        if (data.quantity > 0) {
                            sum += parseFloat(data.original_price) * data.quantity
                        }
                    })
                }
            ...
            const updatePromoQuantity = (action, personType) => {
                ...
                const maxPerson = parseInt(promoData.max_person) || 200;
