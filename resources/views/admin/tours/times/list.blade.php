<div class="table-container universal-table">
    <div class="custom-sec">
        <form id="bulkActionForm" method="POST" action="{{ route('admin.bulk-actions', ['resource' => 'tour-times']) }}">
            @csrf
            <div class="row mb-3">
                <div class="col-md-7">
                    ...
                    <tbody>
                        @foreach ($times as $time)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('admin.tour-times.edit', $time->id) }}" class="link">
                                        {{ $time->name }}
                                    </a>
                                </td>
                                <td class="p-0">
                                    <span
                                        class="badge rounded-pill bg-{{ $time->status == 'publish' ? 'success' : 'warning' }}">
                                        {{ $time->status }}
                                    </span>
                                </td>
                                <td>{{ formatDateTime($time->created_at) }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
